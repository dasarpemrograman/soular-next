-- =====================================================
-- QUICK APPLY: Subscriptions + Premium Film
-- =====================================================
...
-- Update profile subscription when user_subscriptions changes
...
CREATE OR REPLACE FUNCTION has_premium_access(p_user_id UUID, p_required_plan TEXT)
RETURNS BOOLEAN AS $$
DECLARE
  user_plan TEXT;
...
BEGIN
  SELECT subscription_plan INTO user_plan FROM public.profiles WHERE id = p_user_id;
...
CREATE OR REPLACE FUNCTION is_premium_content(p_content_type TEXT, p_content_id UUID)
RETURNS TABLE (is_premium BOOLEAN, required_plan TEXT, preview_duration INTEGER) AS $$
BEGIN
  RETURN QUERY
  SELECT true as is_premium, pc.required_plan, pc.preview_duration
  FROM public.premium_content pc
  WHERE pc.content_type = p_content_type AND pc.content_id = p_content_id
  LIMIT 1;
...
RETURNS TABLE (plan_name TEXT, active_subscribers BIGINT, monthly_revenue DECIMAL, yearly_revenue DECIMAL) AS $$
BEGIN
  RETURN QUERY
  SELECT
    sp.name,
...
-- =====================================================
-- STEP 9: Insert Subscription Plans
-- =====================================================
...
-- =====================================================
-- STEP 10: Add Premium Film
-- =====================================================

INSERT INTO public.films (
  title, slug, description, director, year, duration, category, youtube_url, thumbnail, is_premium, is_published, rating
)
VALUES (
  'Sang Pencari Cahaya',
  'sang-pencari-cahaya',
...
-- =====================================================
-- STEP 11: Mark Film as Premium Content
-- =====================================================

DO $$
DECLARE v_film_id UUID;
BEGIN
  SELECT id INTO v_film_id FROM public.films WHERE slug = 'sang-pencari-cahaya';

  INSERT INTO public.premium_content (content_type, content_id, required_plan, preview_duration, is_early_access)
  VALUES ('film', v_film_id, 'premium', 180, false)
...
SELECT '📊 Plans available:' as info, COUNT(*) as count FROM public.subscription_plans;
SELECT '🎬 Premium films:' as info, COUNT(*) as count FROM public.films WHERE is_premium = true;
SELECT '🔐 Premium content rules:' as info, COUNT(*) as count FROM public.premium_content;
